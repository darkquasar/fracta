---
title: Setup
description: Wire Notion and Readwise to the fracta gateway via native OAuth, stand up the concept extractors, provision the three Notion databases, and verify the stack — about 20 minutes.
---
...
<Tabs>
  <Tab title="Agent-driven (recommended)">
    The OAuth grant you completed in Step 1 gives the fracta gateway the same Notion permissions your user has — including creating new databases. Drive that from any agent CLI with a Notion MCP connection (Claude Code, Codex, OpenCode), or spawn a fracta worker that talks back to your gateway.

    **Via a fracta worker** (uses the gateway's already-configured Notion connection):

    ```bash
    fracta spawn --task provision-reading-garden \
      --contract "$(cat <<'PROMPT'
    Create three Notion databases in my workspace for the fracta Reading Garden pipeline. Use the notion-create-database tool. After each one is created, call notion-fetch on the returned page id and report the data_source_id.

    1) Title: 'Reading Garden — Sources'. Properties:
       - Name (title)
       - readwise_book_id (rich_text)
       - author (rich_text)
       - category (rich_text)
       - source_kind (rich_text)
       - source_url (url)
       - cover_url (url)
       - document_note (rich_text)

    2) Title: 'Reading Garden — Highlights'. Properties:
       - Name (title)
       - readwise_highlight_id (rich_text)
       - note (rich_text)
       - highlighted_at (date)
       - source (relation -> the 'Reading Garden — Sources' database from step 1)

    3) Title: 'Reading Garden — Concepts'. Properties:
       - Name (title)
       - concept_name (rich_text)
       - epistemic_status (select: seedling, budding, evergreen)
       - confidence (number)
       - mention_count (number)
       - last_updated (date)
       - highlights (relation -> the 'Reading Garden — Highlights' database from step 2)

    Report back the three data_source_id values, labelled sources / highlights / concepts.
    PROMPT
    )"

    fracta peek provision-reading-garden
    ```

    **Via your local agent CLI** (Claude Code / Codex / OpenCode with the Notion MCP wired): paste the same prompt block (the part between the `PROMPT` markers) into your agent session. The agent's Notion MCP must be the hosted `mcp.notion.com/mcp` server (the same one fracta uses), authenticated to the same workspace you ran OAuth against in Step 1.

    The data_source_id values feed straight into the [First Run](/patterns/reading-garden/first-run) prompt as `notion_sources_database_id`, `notion_highlights_database_id`, and `notion_concepts_database_id`.
  </Tab>
  <Tab title="Manual (Notion UI)">
    Create three databases by hand, then read back their data_source_ids via a `notion-search` + `notion-fetch` call.

    <Steps>
      <Step title="Create the Sources database">
        In Notion, create a new database titled "Reading Garden — Sources" with these properties (default `Name` plus):
      ...
      <Step title="Create the Highlights database">
        ...
        - `note` (text)
        - `highlighted_at` (date)
        - `source` (**RELATION** -> Sources database)
      </Step>
      <Step title="Create the Concepts database">
        ...
        - `mention_count` (number)
        - `last_updated` (date)
        - `highlights` (**RELATION** -> Highlights database)
      </Step>
      <Step title="Capture the three data_source_id values">
        ...
        Paste these into the [First Run](/patterns/reading-garden/first-run) prompt as `notion_sources_database_id`, `notion_highlights_database_id`, and `notion_concepts_database_id`.
      </Step>
    </Steps>
  </Tab>
</Tabs>
