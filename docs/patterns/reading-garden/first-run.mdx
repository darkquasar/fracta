...
Replace `CONCEPTS_DS_ID`, `HIGHLIGHTS_DS_ID`, and `SOURCES_DS_ID` with the three `data_source_id` values you captured in [Setup — Step 2](/patterns/reading-garden/setup#step-2--provision-the-three-notion-databases). Adjust the `watermark_iso` to whatever cutoff date you want — keeping it recent on the first run avoids long Readwise pulls (the API caps at 20 requests/minute).
