...
<Steps>
  ...
  <Step title="Complete pattern Setup">
    Walk through [Setup](/patterns/reading-garden/setup) once — it covers the Notion + Readwise OAuth dance (via `mcp-remote`), the `fracta.yaml` blocks for those two servers, and the catalog gotchas (Notion DB sharing, Readwise rate limits, plaintext token cache). The current page does not repeat that material.
  </Step>
</Steps>
